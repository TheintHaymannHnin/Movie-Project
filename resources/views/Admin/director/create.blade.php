...
<div class="card mt-3">
        <div class="card-header">
            <h3><b>Create Director</b></h3>
        </div>
            <div class="card-body">
                    <form action="{{url('Admin/directors')}}" method="POST">
                            {{ csrf_field() }}
                              
                            <div class="form-group">
                                <label for="name"><b>Name</b></label>
                                   
                                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="Enter director's name" value="{{old('name')}}">
                                @error('name')
