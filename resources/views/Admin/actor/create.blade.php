...
<div class="card mt-3">
        <div class="card-header">
            <h3><b>Create Actor</b></h3>
        </div>
            <div class="card-body">
                    <form action="{{url('Admin/actors')}}" method="POST">
                            {{ csrf_field() }}
                              
                            <div class="form-group">
                                <label for="name"><b>Name</b></label>
                                   
                                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="Enter actor's name" value="{{old('name')}}">
                                @error('name')
